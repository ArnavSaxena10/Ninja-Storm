...
var side1, side2, side3, side4, flag=0, winImg, win, viperDeath, gameOver, gameOverImg, restartButton, restartButtonImg, viperStill;
var gameOverSound, youWinSound, viperSlashing, kaboonaSlashing, vs2, ks2, bgSound;

function preload(){
  viperAttack=loadAnimation("HeroImages/walk_attack3.png","HeroImages/walk_attack4.png","HeroImages/walk_attack5.png");
  viperWalk=loadAnimation("HeroImages/walk2.png","HeroImages/walk3.png","HeroImages/walk4.png","HeroImages/walk5.png","HeroImages/walk6.png");
  ...
  viperStill=loadImage("HeroImages/walk1.png");
  ...
  youWinSound=loadSound("sounds/you win 1.mp3");
  viperSlashing=loadSound("sounds/slashing.mp3");
  kaboonaSlashing=loadSound("sounds/slashing2.mp3");
  vs2=loadSound("sounds/viper.mp3");
  ks2=loadSound("sounds/kaboona.mp3");
  bgSound=createAudio("sounds/bgMusic.mp3");
...
if(gameState===1){
  ...
  fill("red");
  textSize(25);
  text("INSTRUCTIONS",1235,60);

  fill("orange");
  textSize(25);
  text("Space Bar -> Attack",1220,95);
  text("E -> Special Attack",1220,125);
  ...
  //Viper moves
  if(keyIsDown(32) && kaboona.sprite.x-viper.sprite.x<50 && viper.sprite.y-kaboona.sprite.y===0 && frameCount%20===0){
    healthKaboona=healthKaboona-1;
    viper.sprite.changeAnimation("attack",viperAttack);
    viperSlashing.play();
  }

  if(keyWentUp(32)){
    viper.sprite.changeAnimation("still",viperStill);
  }

  if(keyIsDown(69) && kaboona.sprite.x-viper.sprite.x<50 && viper.sprite.y-kaboona.sprite.y===0 && frameCount%50===0){
  healthKaboona=healthKaboona-10;
  ...
  //kaboona moves
  if(kaboona.sprite.x-viper.sprite.x<50 && viper.sprite.y-kaboona.sprite.y===0 && frameCount%30===0){
  healthViper=healthViper-1.5;
  kaboona.sprite.changeAnimation("attack",kaboonaAttack);
  kaboonaSlashing.play();
  }

  if(kaboona.sprite.x-viper.sprite.x<40 && viper.sprite.y-kaboona.sprite.y===0 && frameCount%35===0){
  healthViper=healthViper-6;
